...
print('free evolution starts at:',datetime.datetime.now())
for i in range(num_samples):
    if i%4000==0:
        print(i)
    evolve_ensemble(dt, sampling_period,
...
trajectories = np.array(trajectories)
print('free evolution ends at:',datetime.datetime.now())
...
nu_name = 'nu_axis_freq='+str(freq)+'.npy'
psd_name= 'psd_freq='+str(freq)+'.npy'
np.save(nu_name, nu_axis)
np.save(psd_name, psd)
#
...
# spec_name = 'z_spec_freq='+str(freq)+'.pdf'
...
# plt.gcf().set_size_inches([default_width, default_height])
# plt.subplots_adjust(left=0.2, right=0.97, top=0.95, bottom=0.2)
# spec_detail_name = 'z_spec_detail_freq='+str(freq)+'.pdf'
# plt.savefig(spec_detail_name)
